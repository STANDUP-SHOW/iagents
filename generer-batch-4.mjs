...
// Agents AG-1175 à AG-1256
const agentsBatch4 = catalogueRaw.agents
  .filter(a => {
    const num = parseInt(a.id.slice(3));
    return num >= 1175 && num <= 1256;
  })
...
async function generer() {
  console.log(`Génération du Batch 4 (${agentsBatch4.length} fiches) en cours...\n`);

  const parSecteur = {};
  for (const agent of agentsBatch4) {
  ...
  console.log(`\n✓ ${compteur} fiches du Batch 4 générées avec succès`);
}
