...
import { tientSur } from './calculer.ts';
...
// 2. No power without a machine to borrow it from, and none on the commander.
for (const o of OFFRES) {
  if (o.role === 'puissance') { const m = machineDe(o); assert.ok(m && m.role === 'bundle', `${o.id} : capacite empruntee a une machine de calcul du releve`); }
  else assert.equal(o.capaciteDe, null, `${o.id} ne porte aucune puissance`);
}
ok('la capacite de chaque machine de puissance vient d une machine du releve ; ni « sans machine » ni le commandeur n en ont');
...
ok('financement : 2 400 € sur 24 mois = 100 €/mois a 0 %, 106,37 €/mois a 6 %');
...
// 6. The ladder only goes up between power machines: every fiche one holds, the next holds too.
// Box Max -> power machine 1 is REPORTED, not refused, while specs are provisional: a 64 Go
// unified-memory box holds analysis agents (19 Go of models) that a 16 Go card does not.
// That is a question for max (power machine 1 needs >= 24 Go of graphics memory), not a bug.
...
for (let i = 2; i < echelle.length; i++) for (const p of tous) if (tientSur(echelle[i - 1], [p])) assert.ok(tientSur(echelle[i], [p]), `${p.id} tient sur ${echelle[i - 1].id} mais pas sur ${echelle[i].id}`);
assert.equal(tenues[3], tous.length, 'la plus grande machine tient chaque fiche seule');
const perdues = tous.filter((p) => tientSur(echelle[0], [p]) && !tientSur(echelle[1], [p]));
ok(`l echelle monte de la machine de puissance 1 a la 3 : ${tenues.slice(1).join(' ⊂ ')} fiches tenues seules (Box Max : ${tenues[0]})`);
if (perdues.length) console.log(`  attention  ${perdues.length} fiches tiennent sur la Box Max et pas sur la machine de puissance 1 (${echelle[1].vram} Go graphiques contre ${echelle[0].vram} Go de memoire unifiee), ex. ${perdues.slice(0, 3).map((p) => p.id).join(', ')}`);
