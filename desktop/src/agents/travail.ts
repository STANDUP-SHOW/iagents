 ...
 */
export const FORMATS_ECRITS = ['md', 'txt', 'csv', 'json', 'html', 'xlsx', 'eml', 'docx', 'pdf'] as const;
...
export interface TacheDuJour {
   ...
   * même, mais il réclamera au lieu d'inventer. 4 547 tâches sur 9 233 sont
...
export function travailDuJour(agent: AgentInstalle): TacheDuJour[] {
  return agent.planning.filter((t) => t.active).map((tache) => {
    ...
    // il réclamera au lieu d'inventer. 4 547 tâches sur 9 233 sont dans ce cas.
