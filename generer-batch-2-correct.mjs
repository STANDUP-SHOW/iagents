...
// Agents AG-0990 à AG-1075
const agentsBatch2 = catalogueRaw.agents
  .filter(a => {
    const num = parseInt(a.id.slice(3));
    return num >= 990 && num <= 1075;
  })
...
const LOGICIELS_PAR_SECTEUR = {
  'energie-environnement': ['excel', 'logiciel-comptabilite', 'client-email', 'gestion-documents'],
  'industrie-back-office': ['excel', 'logiciel-comptabilite', 'client-email', 'erp'],
  'edition-recherche-contenu': ['libreoffice', 'client-email', 'systeme-gestion-doc', 'versionning'],
  'gestion-de-patrimoine-services': ['excel', 'logiciel-comptabilite', 'client-email', 'gestion-assets'],
};
...
async function generer() {
  console.log(`Génération du Batch 2 (${agentsBatch2.length} fiches) en cours...\n`);

  const parSecteur = {};
  for (const agent of agentsBatch2) {
